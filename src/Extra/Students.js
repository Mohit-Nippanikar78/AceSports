...
function Students() {
  ...
  return (
    <>
      <div className="text-lg text-center bg-teal-500 font-extrabold">
        Add Note
      </div>
        ...
        <form class="bg-white w-full shadow-md rounded px-8 pt-6 pb-8 mb-4">
          ...
          <div class="flex items-center justify-between">
            <button
              ...
              type="button"
              onClick={() => {
                Axios.post("http://localhost:3001/students/add", info)
                  .then((res) => {
                    console.log(res);
                    setInfo({
                      name: "",
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mx-4">
        {/* {students?.map((note, i) => {
          return <Student ={note} key={i} fetchNotes={fetchNotes} />;
        })} */}
      </div>
    </>
  );
}
const Student = ({ note, fetchNotes }) => {
